...
function Window({ children, name, scroll = false }: WindowProps) {
  ...
  return createPortal(
    ...
    >
      <div
        className={`fixed top-1/2 left-1/2 bg-gray-50 dark:bg-gray-800 rounded-lg py-12 px-16 transition-all -translate-x-1/2 -translate-y-1/2 ${
          scroll ? "h-full overflow-y-scroll scrollbar" : ""
        }`}
        ref={ref}
      >
        <button
